...
function signatureItemsSummary(items: SignatureItem[], type: RequestType): string {
  ...
  return `
    <div style="background:#f8f9fa;border-radius:6px;padding:16px;margin:16px 0;font-size:14px;color:#444;">
      ...
      <p style="margin:0;color:#666;font-size:13px;">Nomes: ${items.slice(0, 5).map((i) => i.name).join(', ')}${items.length > 5 ? ` e mais ${items.length - 5}…` : ''}</p>
    </div>`
}

// ─── Manager: link de aprovação ───────────────────────────────────────────────

export async function sendManagerApprovalEmail(opts: {
  managerEmail: string
...
export async function sendRequesterApprovedEmail(opts: {
  requesterName: string
  requesterEmail: string
  requestId: string
  decidedBy: string
...
export async function sendRequesterRejectedEmail(opts: {
  requesterName: string
  requesterEmail: string
  reason: string
  companyName?: string
