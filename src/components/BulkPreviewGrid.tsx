				...
				{previews.map((preview) => (
					<Card key={preview.index} className="overflow-hidden">
						<CardContent className="p-3">
							<p className="text-xs font-medium text-muted-foreground mb-2">{preview.data.name}</p>
							<div className="bg-white rounded border p-2 overflow-hidden" style={{ transform: "scale(0.6)", transformOrigin: "top left", height: "auto" }} dangerouslySetInnerHTML={{ __html: preview.html }} />
						</CardContent>
					</Card>
				))}
			</div>
