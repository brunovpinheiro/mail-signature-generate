...
export function ApprovalPage() {
  ...
  // ── Loading ───────────────────────────────────────────────────────────────
  if (loading) {
    return <PageShell><div className="flex items-center gap-2 text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin" /> Carregando…</div></PageShell>
  }
        ...
        <Card className="max-w-lg w-full">
          <CardContent className="pt-6 space-y-4 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {fetchError ?? 'Este link de aprovação não é válido.'}
            </p>
          </CardContent>
        </Card>
      </PageShell>
      ...
      <PageShell>
        <Card className="max-w-lg w-full">
          <CardContent className="pt-6 space-y-4 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Esta solicitação já foi <strong>{label}</strong> por outro gestor.<br />
      ...
      <PageShell>
        <Card className="max-w-lg w-full">
          <CardContent className="pt-6 space-y-4 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              O solicitante foi notificado por e-mail.<br />
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="rounded-md bg-muted/50 p-3 text-sm space-y-1">
              <p><span className="text-muted-foreground">Solicitante:</span> <strong>{requestData.requesterName}</strong></p>
          ...
          <CardContent className="space-y-4">
            {requestData.signatureItems.map((item: SignatureData, i: number) => (
              <div key={i} className="rounded-md border bg-white p-4">
                {requestData.type === 'bulk' && (
                  <p className="text-xs text-muted-foreground mb-2 font-medium">{item.name}</p>
                )}
                {template && (
                  <div
                    className="overflow-x-auto"
                    dangerouslySetInnerHTML={{ __html: template.render(item) }}
                  />
                )}
              </div>
            ))}
        ...
        <Card>
          <CardContent className="pt-6 space-y-4">
            ...
            <div className="flex gap-3">
              ...
              {action !== null && (
                <Button variant="ghost" size="lg" onClick={() => { setAction(null); setReason(''); setReasonError(false) }} disabled={submitting}>
                  Cancelar
                </Button>
              )}
            </div>
            ...
            {action === 'approve' && (
              <p className="text-xs text-center text-muted-foreground">
                Clique em "Confirmar Aprovação" para aprovar a assinatura.
              </p>
            )}
          </CardContent>
        </Card>
